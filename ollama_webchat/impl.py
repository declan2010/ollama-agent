                                ...
                                if followup_content and not followup_tool_calls:
                                    ...
                                    if followup_tool_calls and tools_for_this_round is None:
                                        ...
                                        if content2:
                                            ...
                                            break
                                    # If we have tool results but no final text response, show the tool results
                                    if not full_response and followup_tool_calls:
                                        tool_summaries = []
                                        for m in followup_messages:
                                            if m.get('role') == 'tool' and m.get('content'):
                                                tool_summaries.append(m['content'][:500])
                                        if tool_summaries:
                                            full_response = "📋 **Resultados de herramientas:**\n\n" + "\n\n---\n\n".join(tool_summaries)
                                            yield f"data: {json.dumps({'type': 'token', 'content': full_response})}\n\n"
                                        else:
                                        ...
                                        # Keep what we have
                                        yield f"data: {json.dumps({'type': 'token', 'content': '\n\n_(El modelo usó herramientas pero no completó su respuesta)_'})}\n\n"
                                    else:
                                        full_response = "(No response from model)"
                                        yield f"data: {json.dumps({'type': 'token', 'content': full_response})}\n\n"
                                    break
